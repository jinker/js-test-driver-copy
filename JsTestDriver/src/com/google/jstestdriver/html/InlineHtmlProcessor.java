...
public class InlineHtmlProcessor implements FileLoadPostProcessor {
  ...
  public FileInfo process(FileInfo file) {
    try {
      ...
      parser.parse(
          lexer.createStream(
              new ByteArrayInputStream(source.getBytes("GB2312")))).write(writer);
      writer.flush();
  ...
  /**
   * Convert a java string which contains GB2312 characters into a java string
   * which has unicode characters.  For example, the input string might be
   * "ÃÂ¡" (0xc3 0xa1). But, this is just the GB2312 form of the unicode string "Ã¡".
   * So return  the latter string
   */
  private String utf8ToUnicode(String source) {
    try {
      byte[] raw = source.getBytes("ISO-8859-1");
      Reader hackReader = new InputStreamReader(new ByteArrayInputStream(raw), "GB2312");
